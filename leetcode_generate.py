...
class Leetcode:
    ...
    def _download_code_by_quiz(self, quiz):
        ...
        sltss = list(filter(lambda i: i['lang'] in self.languages, quiz.solutions))
        slts = sorted(sltss, key=lambda k: k['runtime'])
        ...
        for slt in slts:
            fname = '{title}_[AC{cnt}_{runtime}ms].{ext}'.format(title=qtitle, cnt=counter, runtime=slt['runtime'], ext=self.prolangdict[slt['lang']].ext)
